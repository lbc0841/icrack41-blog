...
最短重複子字串長度 `l` = `s.size()` - `kmp.back()`<br>
最大重複次數 = `s.size()` / `l`

驗證 `s.size()` / `l` 是否能整除<br>
否則最大重複次數 = 1
...
窮舉重複字串可能的長度<br>
只有長度是 `s.size()` 因數的 **重複字串長度** 才能讓 **字串重複次數** 是整數<br>
...
`s.size() = 8`<br>
可能的 **重複字串的長度** `l` 為 1, 2, 4<br>
如果假設正確 `s[i]` 經過 `l` 個字符後會再重複一次 (還會是`s[i]`)<br>
